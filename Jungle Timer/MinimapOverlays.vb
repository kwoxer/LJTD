Public Class MinimapOverlays
    ...
    Public Sub New(ByVal scalex As Single, ByVal scaley As Single, ByVal duration As TimeSpan, ByVal buffID As Integer)
        ...
        picBoxDuration = New Label With {
            ...
            .Font = New System.Drawing.Font(resource.PropFont(0, 1), resource.PropMinimapInt(9), FontStyle.Bold),
    ...
    Private Sub Image_Load()
        ...
        Catch ex As Exception
            Module_Logfile.Logfile_Append("Some images are not available.")
        End Try
    End Sub
    ''' <summary>
    ...
    Public Sub Objective_Tick(ByVal buffID As Integer)
        If running = True Then
            ...
            End If
            If resource.PropRemember(0, 1) = actualShownTimeMin Then
                ...
                If resource.PropConfigBool(9) = True Then
                    If resource.PropConfig(25, 1) = "Ping" Then
                        ObjectiveSound_Play(True, 0)
                    Else
                        ObjectiveSound_Play(False, (buffID * 3) + 1)
                    End If
                End If
                If ObjectiveStatus_Check(buffID) Then
                    If resource.PropRememberBool(3) Then
                        LJTD.OpenerValue_Set(resource.PropConfig(2, 1))
                        Module_Write2Chat.Write2Chat_Send(LJTD.Objective(buffID).GetName & resource.PropRemember(6, 1) & resource.PropRemember(0, 1), True)
                    End If
                End If
            ElseIf resource.PropRemember(1, 1) = actualShownTimeMin Then
                ...
                If resource.PropConfigBool(9) = True Then
                    If resource.PropConfig(25, 1) = "Ping" Then
                        ObjectiveSound_Play(True, 0)
                    Else
                        ObjectiveSound_Play(False, (buffID * 3) + 3)
                    End If
                End If
                If ObjectiveStatus_Check(buffID) Then
                    If resource.PropRememberBool(4) Then
                        LJTD.OpenerValue_Set(resource.PropConfig(2, 1))
                        Module_Write2Chat.Write2Chat_Send(LJTD.Objective(buffID).GetName & resource.PropRemember(6, 1) & resource.PropRemember(1, 1), True)
                    End If
                End If
            ElseIf resource.PropRemember(2, 1) = actualShownTimeMin Then
                ...
                If resource.PropConfigBool(9) = True Then
                    If resource.PropConfig(25, 1) = "Ping" Then
                        ObjectiveSound_Play(True, 0)
                    Else
                        ObjectiveSound_Play(False, (buffID * 3) + 2)
                    End If
                End If
                If ObjectiveStatus_Check(buffID) Then
                    If resource.PropRememberBool(5) Then
                        LJTD.OpenerValue_Set(resource.PropConfig(2, 1))
                        Module_Write2Chat.Write2Chat_Send(LJTD.Objective(buffID).GetName & resource.PropRemember(6, 1) & resource.PropRemember(2, 1), True)
                    End If
                End If
            End If
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub ObjectiveSound_Play(usePingSound As Boolean, i As Integer)
        If resource.PropConfigBool(9) Then
            Try
                If usePingSound Then
                    My.Computer.Audio.Play(System.IO.Path.GetFullPath(resource.PropSound(0)), AudioPlayMode.Background)
                Else
                    MP3_Stop(i)
                    MP3_Play(resource.PropSound(i), i)
                End If
            Catch ex As Exception
                Module_Logfile.Logfile_Append("An audio file cound not be found.")
            End Try
        End If
    End Sub
    ''' <summary>
    ''' Checks if the specific event is a running Objective
    ''' </summary>
    ''' <param name="buffID"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function ObjectiveStatus_Check(buffID As Integer) As Boolean
        If buffID = 0 Or buffID = 1 Then
            If resource.PropWrite2ChatBool(0) Then
                Return True
            End If
        ElseIf buffID = 2 Or buffID = 3 Or buffID = 4 Or buffID = 5 Then
            If resource.PropWrite2ChatBool(1) Then
                Return True
            End If
        End If
        Return False
